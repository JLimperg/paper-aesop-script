...
\begin{algorithm}
  \begin{algorithmic}
    ...
    \Function{next-step}{$\mathit{tacticState}, \mathit{steps}$}
      ...
      \Else
        \State \Return the first step whose pre-goal is contained
        \State \hspace{\widthof{\Return}} \cindent in $\mathit{tacticState}$
      \EndIf
    \EndFunction
    ...
    \Function{apply-step}{$\mathit{tacticState}, (Φ, g, t, Ψ, G)$}
      ...
      \State Remove from $r$ each goal $x$ such that $x$ is assigned in
  ...
  \begin{algorithmic}
    ...
    \Function{apply-step}{$\mathit{steps}, Φ, G, g, s$}
      ...
      \If{$(Φ^{\post}, G^{\post}) \aeq (Ψ, G')$}
        ...
        \State For each step in $\mathit{steps}$, replace its pre-goal $g^{\pre}$
        \State \cindent with $σ(g^{\pre})$ if $g^{\pre} ∈ \dom(σ)$
