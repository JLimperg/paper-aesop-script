...
\begin{algorithm}
  \begin{algorithmic}
    ...
    \Function{apply-step}{$\mathit{steps}, Φ, G, g, s$}
      ...
      \State Run $t$ in tactic state $(Φ, [g])$, producing tactic state $(Ψ, G')$
      \If{$(Φ^{\post}, G^{\post}) \aeq (Ψ, G')$}
        \State \Let $σ$ be the partial map from metavariables in $Φ^{\post}$ to matching metavariables in $Ψ$
        \State For each step in $\mathit{steps}$, replace its pre-goal $g^{\pre}$ with $σ(g^{\pre})$ if $g^{\pre} ∈ \dom(σ)$
