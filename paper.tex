...
\newcommand{\ty}[1]{\ensuremath{\mathrm{#1}}}
...
\acmDOI{10.1145/3703595.3705877}
\acmYear{2025}
\copyrightyear{2025}
\acmISBN{979-8-4007-1347-7/25/01}
\acmConference[CPP '25]{Proceedings of the 14th ACM SIGPLAN International Conference on Certified Programs and Proofs}{January 20--21, 2025}{Denver, CO, USA}
\acmBooktitle{Proceedings of the 14th ACM SIGPLAN International Conference on Certified Programs and Proofs (CPP '25), January 20--21, 2025, Denver, CO, USA}
\received{2024-09-17}
\received[accepted]{2024-11-19}
...
\keywords{tactic, Lean, interactive theorem proving, proof automation, proof search}

\begin{document}

\begin{abstract}
  White-box proof search tactics such as Coq's \tac{auto}, Isabelle's \tac{auto} and Lean's Aesop apply proof rules that often translate directly to lower-level tactics.
  ...
  \begin{subfigure}[b]{0.35\textwidth}
    \[
      \begin{array}{l}
        ...
        \tac{simp\_all} \\
        \tac{split} \\
        \tac{rename\_i}~\mathit{x\_1}~\mathit{heq} \\
        \tac{on\_goal}~2 ⇒ \tac{rename\_i}~\mathit{x\_1}~a~\mathit{as}~\mathit{heq} \\
        \tac{simp\_all} \\
        \tac{split} \\
        \tac{rename\_i}~\mathit{x\_2} \\
        \tac{on\_goal}~2 ⇒ \tac{rename\_i}~\mathit{x\_2}~\mathit{a\_1}~\mathit{as\_1} \\
        \tac{subst}~\mathit{heq} \\
        \tac{simp\_all} \\
        \tac{simp\_all}
      \end{array}
  ...
  \begin{subfigure}[b]{0.35\textwidth}
    \[
      \begin{array}{l}
        ...
        \tac{split} \\
        \tac{next}~\mathit{x\_1}~\mathit{heq} ⇒ \\
        \quad \tac{simp\_all~only}~[\mathrm{append\_eq\_nil}, \mathrm{false\_and}] \\
        \tac{next}~\mathit{x\_1}~a~\mathit{as}~\mathit{heq} ⇒ \\
        \quad \tac{split} \\
        \quad \tac{next}~\mathit{x\_2} ⇒ \\
        \quad \quad \tac{subst}~\mathit{heq} \\
        \quad \quad \tac{simp\_all~only}~[\mathrm{not\_true\_eq\_false}] \\
        \quad \tac{next}~\mathit{x\_2}~\mathit{a\_1}~\mathit{as\_1} ⇒ \\
...
First, it sometimes works on the goals out of order, using the combinator \tac{on\_goal~2} to run a tactic on the second goal in the current list of goals.
...
Since this transformation also speeds up \tac{simp\_all}, it is applied to all invocations, not just the non-terminal ones.
...
The same approach is also used when the optimiser replaces \tac{simp\_all} invocations with \tac{simp\_all~only} as in Fig.~\ref{fig:big-example}.
...
When the rule generates these tactic invocations, it does not yet know which other tactics will be used to eventually close the first subgoal, but it already needs to operate on the other subgoals.
...
\begin{algorithm}
  \begin{algorithmic}
    ...
    \Function{apply-step}{$\mathit{tacticState}, (Φ, g, t, Ψ, G)$}
      ...
      \State Replace $g$ with $g₁, \dots, gₙ$ in $\mathit{tacticState}$
      \State Remove from $\mathit{tacticState}$ each goal $g'$ such that $g'$
      \State \cindent is assigned in $Ψ$ but not in $Φ$
      \State \Return $\mathit{tacticState}$
    \EndFunction
  \end{algorithmic}
...
Hence, \textsc{apply-step} fails only when the input metavariable context $Φ$ differs from $Φ^{\pre}$.
This can happen when \textsc{apply-step} is called after we have reordered some steps, i.e.\ after at least one call to \textsc{go-structured}.
But it cannot happen when we execute the steps in the original order.
As a result, \textsc{reorder-dynamic}, which in the worst case falls back to a sequence of \textsc{go-unstructured} calls, cannot fail.
...
We assume that a step tree for $\mathit{steps}$ has been constructed previously, and that we can therefore determine the number of siblings of a step $s ∈ \mathit{steps}$, as well as whether $s$ is focusable (and if so, what its maximal child index is).

The \textsc{sscript} function then returns a new tactic state and a structured script.
