...
In interactive theorem provers, white-box proof automation tactics such as Coq's (TODO ref) auto, Isabelle's (TODO ref) auto (TODO ref) and Lean's (TODO ref) Aesop (TODO ref) provide a simple but effective and customisable way to automate straightforward proofs.
...
\footnote{This terminology is only loosely related to structured proof languages such as Isar (TODO ref).}
...
(This is also the reason why Aesop's proof tree cannot, in general, store the rules in depth-first order~\cite[Section~TODO]{TODO:aesop}.)
...
The described optimisation pipeline is available in mainline Aesop\footnote{\url{https://github.com/leanprover-community/aesop}} and can already be used by Mathlib (TODO ref) developers and other Lean users.
...
Lean exposes an extensive metaprogramming API (TODO ref) that is used to develop the Lean compiler itself as well as user-defined tactics such as Aesop.
...
We can therefore also view a tactic as a function of \emph{tactic contexts}.
A tactic context $(Φ, G)$ contains a metavariable context $Φ$ and a list of goals $G$.
When a tactic is executed with $(Φ, G)$ as input, it produces another tactic context $(Ψ, H)$ as output, where $H$ contains the output goals and $Ψ$ is the metavariable context derived from $Φ$ by adding the new output goals and assigning proof terms to input goals.
...
When the tactic corresponding to a rule is run in a tactic context $(Φ,\allowbreak [g₁, \dots, gₙ])$, it receives the first goal $g₁$ as input, creates subgoals $h₁, \dots, hₘ$ (where $m$ can be zero) and returns a tactic context with goals $h₁, \dots, hₘ, g₂, \dots gₙ$.
...
Many of our optimisations will need to check whether two tactics $t₁$ and $t₂$, when run in the same tactic context $(Φ, G)$, produce the same result.
...
Its main function, \textsc{reorder-static}, first sets up the \emph{tactic state}.
This is the list of current proof goals, containing initially the pre-goal of the first step.
We then iteratively select steps and apply them to the tactic state (simulating the effect of the step's tactic) until the tactic state becomes empty and the proof is complete.
...
To evaluate the performance of script generation, we build Mathlib\footnote{Commit TODO, committed on TODO.} (TODO ref), Lean's mathematical library, in single-threaded mode with an option that makes Aesop always generate (but not display) a script.
...
Huang et al.\ (TODO ref) translate resolution proofs into natural deduction proofs (and later natural language proofs).
In particular, they compress multiple natural deduction steps into one \enquote{assertion-level} step that applies a theorem or definition (as a human would).

Pąk (TODO ref) optimises Mizar proofs by extracting lemmas for common deduction steps and by removing unnecessary references, assumptions, and so on.
...
Vyskocil, Stanovský and Urban (TODO ref) also compress proofs via lemma extraction.
A variety of tools used to improve Mizar proofs is described by Grabowski and Schwarzweller (TODO ref).
...
However, there have also been attempts to directly transform the ATP proof into the proof assistant's language, in Isabelle (TODO ref), Mizar (TODO ref) and HOL Light (TODO ref).
